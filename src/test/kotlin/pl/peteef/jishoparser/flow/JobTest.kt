...
import pl.peteef.jishoparser.test.pageEntity
...
internal class JobTest {
    init {
        ...
        every { HttpClient.get(any()) }.returns(listOf(pageEntity))
        mockkObject(Processor)
    }

    @Test
    fun itShouldPerformJob() {
        val job = Job(SearchCriteria(N5), ProcessingSettings("filename", JSON_TO_FILE))
        job.perform()
